...
当前完整 mesh 模型在 RTX 4060 Laptop 上、8192 环境、预热 10 步后采样 48 步的
严格配对短测如下。原始 `nconmax=200, njmax=200` 为 **97.99 ms/控制步（8.36 万
环境步/秒）**；将接触容量改为 `nconmax=64`、保留 `njmax=200` 后为 **90.87 ms/
控制步（9.02 万环境步/秒）**，采样吞吐提高约 **7.8%**。平地和跳跃压力测试的
单环境接触峰值分别不超过 16 和 15，均低于 64；长测也保持有限值。独立降低
`njmax` 没有可重复收益，因此默认值仍为 200。

PPO 侧会在同一批 actor/critic 计算中复用一次冻结的历史编码结果，避免重复执行
encoder；这不改变网络、损失或梯度路径。默认仍执行 5 个 PPO epoch。降低到 3 个
epoch 和更小的时序网络虽然短测更快，但会改变更新比率或模型容量，只保留在实验
分支中，不作为默认训练设置。完整实验记录见
[训练加速实验](docs/training-speed-experiments.md)。
...
在基准中独立测试配置，命令行覆盖不会改写默认训练配置。输出包含单环境接触峰值、
全局接触容量和约束峰值，用于检查缓冲区余量。减小缓冲区必须检查接触/约束溢出，
放宽容差或减少迭代还必须验证闭链误差与训练效果。
